...
// import { faCalendar, faUser, faArrowLeft, faTrash, faPaperPlane } from '@fortawesome/free-solid-svg-icons';
...
//       const cRes = await axios.get(`/api/comments?type=article&parentId=${id}`);
...
//     try {
//       await axios.post('/api/comments', { type: 'article', parentId: id, message: newComment });
...
//   if (!article) return <div className="py-32 text-center"><p className="text-stone-500">Article non trouvé</p></div>;

//   return (
//     <div className="py-16">
//       <div className="container mx-auto px-4 max-w-3xl">
//         <Link href={`/${locale}/blog`} className="text-primary-500 hover:text-primary-600 flex items-center gap-1 mb-8">
//           <FontAwesomeIcon icon={faArrowLeft} /> Retour aux actualités
//         </Link>

//         <motion.article className="card overflow-hidden mb-8" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
//           {article.image && <img src={article.image} alt="" className="w-full h-64 object-cover rounded-t-xl" />}
//           <div className="p-8">
//             <div className="flex items-center gap-4 text-sm text-stone-400 mb-4">
//               <span><FontAwesomeIcon icon={faCalendar} className="mr-1 text-primary-400" />{new Date(article.createdAt).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
//               <span><FontAwesomeIcon icon={faUser} className="mr-1 text-primary-400" />{article.authorName || 'AYSA Foundation'}</span>
//             </div>
//             <h1 className="text-3xl font-heading font-bold mb-6">{article.title?.fr || article.title?.en}</h1>
//             <div className="prose prose-stone max-w-none" dangerouslySetInnerHTML={{ __html: article.content?.fr || article.content?.en || '' }} />
//           </div>
//         </motion.article>
...
import { motion } from 'framer-motion';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCalendar, faUser, faArrowLeft, faTrash, faPaperPlane, faEllipsisV, faEdit } from '@fortawesome/free-solid-svg-icons';
import axios from 'axios';
import Link from 'next/link';
import toast from 'react-hot-toast';
...
export default function ArticleDetail() {
  ...
  const isOwner = (comment) => {
    return user && (user.id === comment.userId?._id || user.id === comment.userId);
  };

  const isAdmin = () => {
    return user && ['SUPER_ADMIN', 'PRESIDENT', 'ADMIN'].includes(user.role);
  };
  ...
  const handleEditComment = async (commentId) => {
    try {
      await axios.put(`/api/comments/${commentId}`, { message: editText });
      ...
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="card p-8">
          ...
          {user ? (
            ...
            <p className="text-stone-400 mb-8 text-center bg-stone-50 rounded-xl p-4">
              <Link href={`/${locale}/auth`} className="text-primary-500 font-medium">Connectez-vous</Link> pour commenter
            </p>
          )}

          {comments.length === 0 ? (
            <p className="text-center text-stone-400 py-8">Aucun commentaire. Soyez le premier à commenter !</p>
          ) : (
              ...
              {comments.map(c => (
                <div key={c._id} className="flex gap-3 group">
                  ...
                  <div className="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center flex-shrink-0 mt-1">
                    {c.userId?.avatar ? (
                      <img src={c.userId.avatar} className="w-full h-full rounded-full object-cover" alt="" />
                    ) : (
                      <span className="text-sm font-bold text-primary-500">{c.userId?.name?.charAt(0) || 'A'}</span>
                    )}
                  </div>
                    ...
                    <div className="bg-stone-50 rounded-2xl px-4 py-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {canDelete(c) && (
                          <div className="relative opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            >
                              <FontAwesomeIcon icon={faEllipsisV} className="text-stone-400 text-xs" />
                            </button>
                            ...
                            {openMenuId === c._id && (
                              <div className="absolute right-0 top-8 bg-white rounded-lg shadow-lg border border-stone-200 py-1 z-10 min-w-[120px]">
                                {canEdit(c) && (
                                  <button 
                                    onClick={() => { setEditingComment(c._id); setEditText(c.message); setOpenMenuId(null); }}
                                    className="w-full text-left px-4 py-2 text-sm hover:bg-stone-50 flex items-center gap-2"
                                  >
                                    <FontAwesomeIcon icon={faEdit} className="text-stone-400 text-xs" /> Modifier
                                  </button>
                                )}
                                <button 
                                ...
                                >
                                  <FontAwesomeIcon icon={faTrash} className="text-xs" /> Supprimer
                                </button>
                              </div>
                            )}
                          </div>
                      ...
                      {editingComment === c._id ? (
                        <div className="flex gap-2 mt-1">
                          <input value={editText} onChange={e => setEditText(e.target.value)} className="input-field text-sm flex-1" />
                          <button onClick={() => handleEditComment(c._id)} className="text-primary-500 text-sm font-medium">Enregistrer</button>
                          <button onClick={() => setEditingComment(null)} className="text-stone-400 text-sm">Annuler</button>
                        </div>
                      ) : (
