...
import { motion } from 'framer-motion';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faClock, faArrowLeft, faTrash, faPaperPlane, faThumbtack, faUser, faEllipsisV, faEdit } from '@fortawesome/free-solid-svg-icons';
import axios from 'axios';
import Link from 'next/link';
import toast from 'react-hot-toast';
...
export default function AnnouncementDetail() {
  ...
  const handleComment = async (e) => {
    ...
    if (!newComment.trim() || !user) return toast.error('Connectez-vous pour commenter');
    try {
      await axios.post('/api/comments', { type: 'announcement', parentId: id, message: newComment, userId: user.id });
      toast.success('Commentaire ajouté');
      setNewComment('');
      fetchAnnouncement();
    } catch (err) { toast.error('Erreur'); }
  ...
  const handleDeleteComment = async (commentId) => {
    if (!confirm('Supprimer ce commentaire ?')) return;
    try { await axios.delete(`/api/comments/${commentId}`); toast.success('Supprimé'); setOpenMenuId(null); fetchAnnouncement(); }
    catch (err) { toast.error('Erreur'); }
  };

  const handleEditComment = async (commentId) => {
    try { await axios.put(`/api/comments/${commentId}`, { message: editText }); toast.success('Modifié'); setEditingComment(null); setOpenMenuId(null); fetchAnnouncement(); }
    catch (err) { toast.error('Erreur'); }
  };
    ...
    <div className="py-16">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <motion.div className="card overflow-hidden mb-8" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          {announcement.image && <img src={announcement.image} className="w-full h-64 object-cover rounded-t-xl" alt="" />}
          <div className="p-8">
            <div className="flex items-center gap-3 mb-4">
              {announcement.isPinned && <span className="text-xs bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full font-medium"><FontAwesomeIcon icon={faThumbtack} className="mr-1" />Épinglé</span>}
              <span className={`text-xs px-3 py-1 rounded-full font-medium ${
                announcement.type === 'urgent' ? 'bg-red-100 text-red-700' : announcement.type === 'official' ? 'bg-blue-100 text-blue-700' : announcement.type === 'press' ? 'bg-purple-100 text-purple-700' : 'bg-stone-100 text-stone-600'
              }`}>{announcement.type?.toUpperCase()}</span>
            </div>
            <h1 className="text-3xl font-heading font-bold mb-4">{announcement.title?.fr || announcement.title?.en}</h1>
            <p className="text-sm text-stone-400 mb-6"><FontAwesomeIcon icon={faClock} className="mr-1" />{new Date(announcement.createdAt).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
            <div className="prose prose-stone max-w-none" dangerouslySetInnerHTML={{ __html: announcement.content?.fr || announcement.content?.en || '' }} />
          </div>
        </motion.div>
